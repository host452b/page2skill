...
When reusing skills generated by bookmark2skill in future tasks:

### Discovery: Taxonomy → Frontmatter → Content

1. **Read `taxonomy.toml` first** — understand what category directories exist
2. **Scan frontmatter of files in relevant category** — `reuse_contexts.situation` tells you WHEN a skill is relevant
3. **Only read full content when the frontmatter matches your task** — saves context window

### Relevance Matching

Use these frontmatter fields to decide relevance (in priority order):
- `reuse_contexts[].situation` — explicit match: "when doing X, use this"
- `tags[]` — keyword match against your current task
- `key_claims[]` — if the claim supports or contradicts your current approach
- `content_type` — filter by article type (technical-article, tutorial, opinion, etc.)

### Taste Signal Aggregation

Over time, aggregate `taste_signals` across all skills to model user preferences:
- `aesthetic[]` — what style the user gravitates toward (minimalism, depth, etc.)
- `intellectual[]` — how the user thinks (first-principles, empiricism, etc.)
- `values[]` — what the user cares about (pragmatism, anti-complexity, etc.)

Use these aggregated signals to:
- Prioritize approaches the user would prefer
- Avoid patterns the user's taste signals consistently reject
- Frame explanations in the user's preferred thinking style

### Quality-Based Prioritization

When multiple skills match, use `quality_score` to rank:
- `depth` — prefer higher for complex decisions
- `originality` — prefer higher when seeking novel approaches
- `practicality` — prefer higher when implementing
- `writing` — prefer higher when the user needs to read the source

### Batch Processing Strategy

For processing many bookmarks:
1. Run `bookmark2skill list --source chrome --only-new` to get pending URLs
2. Process ONE bookmark at a time: fetch → distill → write → mark
3. After each bookmark, check `bookmark2skill status` to track progress
4. If a fetch fails, `mark-failed` immediately and move to the next
5. Don't retry failed URLs in the same session — they likely need human review

### Error Recovery

- **HTTP 4xx** — `mark-failed`, likely permanent (deleted page, paywall)
- **HTTP 5xx / timeout** — `mark-failed` with reason, may succeed on retry later
- **Empty content after fetch** — page is likely JS-rendered; note for Playwright retry
- **Schema validation error** — fix your structured JSON, don't skip validation
